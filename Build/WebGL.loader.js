function createUnityInstance(canvas, config, onProgress = () => { }) {
  ...
  var Module = {
    ...
    wasmFileSize: 56424905,
    cacheControl: function (url) {
      const isCacheable = (
        url == Module.dataUrl
        || Module.primaryDataUrls && Module.primaryDataUrls.includes(url)
        || Module.secondaryDataUrls && Module.secondaryDataUrls.includes(url)
        || url.match(/\.bundle/)
      );
      return isCacheable ? "must-revalidate" : "no-store";
    },
...
Module.UnityCache = function () {
  ...
  function downloadBinary(url, loadingDeferred = false) {
      ...
      var cacheControl = Module.cacheControl(url);
